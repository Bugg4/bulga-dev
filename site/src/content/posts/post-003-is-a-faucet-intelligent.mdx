---
postNumber: 3
slug: "post-003-is-a-faucet-intelligent"
title: "Is a Faucet intelligent?"
...
draft: false
---
import SpeakerQuote, { Quote } from "../../components/SpeakerQuote";
...
I had dropped [an OpenAI post](https://x.com/OpenAI/status/2097374640582668336) into the group chat with the restrained caption: <Quote>A Millennium problem solved</Quote>. I followed it with the equally restrained claim: anyone who could not see at least the first glimmers of AGI in this had ham over their eyes.

One friend was less interested in taxonomy. AGI, ASI, smarter than humans, <SpeakerQuote speaker="Y">really thinking</SpeakerQuote>: what mattered was that the system was useful and producing results that would have taken us much longer to reach.

Another friend's immediate objection was blunt:

<SpeakerQuote speaker="G">It doesn't really think.</SpeakerQuote>

My instinctive response was the duck test. If <Quote>it walks like a duck and quacks like a duck...</Quote><br />
...
I did not feel like that analogy invalidate my argoument; in fact, it made **scale** precisely where the magic happens.<br />
...
The conversation then took the scenic route through mathematical search, a simple program adding large numbers, a possibly intelligent faucet, gnats, cats, and children.<br />

Eventually I asked whether a single-celled organism was intelligent. What about ten million cells? A tadpole? A cat? A human?

<SpeakerQuote speaker="G">Cardinality alone changes nothing</SpeakerQuote> came the objection. A neuron is not intelligent; neither are a hundred thousand neurons taken individually. Intelligence comes from the interaction of the whole system.
...
<SpeakerQuote speaker="G">No</SpeakerQuote>, came the answer. <SpeakerQuote speaker="G">A plant follows an algorithm encoded in its DNA</SpeakerQuote>.

Fair enough. But then the annoying question: <Quote>how is that fundamentally different from us?</Quote>

We also act according to structures encoded in biology, modified by experience, and executed by matter that follows physical laws. Our overall system is vastly larger and more adaptable, but its local rules may still be simple. Calling the plant <SpeakerQuote speaker="G">just an algorithm</SpeakerQuote> does not tell us where intelligence begins. It only moves the mystery one step up the complexity ladder.
...
Computation is not a property exclusive to silicon. You can encode an algorithm into a [mechanical device](https://en.wikipedia.org/wiki/Mechanical_computer), in [flowing water](https://en.wikipedia.org/wiki/Water_integrator), in [air pressure](https://www.youtube.com/watch?v=E1BLGpE5zH0), in transistors, or in cells. Once a system can receive input, transform information, and produce output, it can execute an algorithm. The substrate changes what is practical, not what computation fundamentally is.
...
<SpeakerQuote speaker="G">Humans can generalize</SpeakerQuote> sounds like a promising answer, until machines start doing it too.
...
The same friend stress-tested this claim with increasingly minimal examples. Is travelling-salesman search intelligent? Tabu search over an arbitrarily large space? A simple program that adds three enormous numbers?

I bit the bullet: <Quote>yes, but very little</Quote>. Extrapolating towards zero, any algorithm actually operating on data has some vanishingly small place on the spectrum. That does not make a three-line adder AGI, any more than a gnat is a human because both are alive. The size of the numbers is not what matters; the scale and variety of problems the system can navigate is.
...
At one point, the same friend offered a stricter definition:<br />
<SpeakerQuote speaker="G">intelligence is the innate ability to generalize, learn, remember, and imagine.</SpeakerQuote><br />
...
Intelligence looks less like a substance a system possesses and more like a relationship between its machinery, its information, and the problems around it.<br />
